...
def get_tiny_images(image_paths):
    ...
    # output_dim means the input image is resized to 10*10
    output_dim = 10
    image_feats = np.empty((len(image_paths), output_dim * output_dim), dtype=float)

    for i in range(0, len(image_paths)):
        image = imread(image_paths[i])
        mdim = min(image.shape)
        # Crop from the middle to retain key information in the image center
        image = image[image.shape[0]//2-mdim//2: image.shape[0]//2+mdim//2,
                      image.shape[1]//2-mdim//2: image.shape[1]//2+mdim//2]
        image = rgb2grey(image)
        # It is crucial to filter when down-sampling the image to avoid aliasing artifacts.
        image_feat = resize(image, (output_dim, output_dim), anti_aliasing=True)
        # Normalizing these tiny images
        image_feat = image_feat / np.sum(image_feat)
        image_feats[i, :] = np.reshape(image_feat, (output_dim * output_dim))
...
def build_vocabulary(image_paths, vocab_size):
    ...
    for i in range(0, len(image_paths)):
        ...
        # L2-norm reduces the influence of light
        hogs = hog(image, orients, cells_per_block=cells_per_block, pixels_per_cell=pixels_per_cell,
                   feature_vector=True, block_norm='L2').reshape(-1, cells_per_block[0] * cells_per_block[1] * orients)
        out = np.concatenate((out, hogs))

    # fit(): Compute the centroids
    # use centroids as the entries in vocabulary
    kmeans = MiniBatchKMeans(vocab_size, max_iter=100, tol=1e-4, random_state=0).fit(out)
...
def nearest_neighbor_classify(train_image_feats, train_labels, test_image_feats):
    ...
    for i in range(0, len(test_image_feats)):
        ...
        mode, _ = stats.mode(nn_label, axis=None)
        labels[i] = mode[0]
