...
def non_max_suppression_bbox(bboxes, confidences, img_size, verbose=False):
    ...
    # np.argsort(): from small to large, if axis == None, the flattened array is used.
...
def voc_ap(rec, prec):
    ...
    # reversed(): returns the reverse iterator
...
def visualize_detections_by_image(bboxes, confidences, image_ids, tp, fp,
    ...
    gt_file_list = list(set(gt_ids))[:20]
...
def visualize_detections_by_image_no_gt(bboxes, confidences, image_ids,
    ...
    :return:
    """
    test_filenames = glob(osp.join(test_scn_path, '*.jpg'))[:30]
